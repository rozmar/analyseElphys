...
% aE_checkGJandChemicalSynapse.m plots figures of synaptic and GJ
% connectiviy o recorded cells. The input variables are valtozok, xlsdata,
% dirs, prenum.
% xlsdata and dirs are generated by analysElphys_main.m script.
% prenum: the indexes of the potential presynaptic cells to check
% valtozok variable contains the user selected variables
% valtozok.plot.dpi=150; - DPI of image
% valtozok.plot.xcm=20; - horizontal size of image
% valtozok.plot.ycm=14;- vertical size of image
% valtozok.plot.betumeret=8; - font size
% valtozok.plot.betutipus='Arial'; - font type
% valtozok.plot.axesvastagsag=2; - axes width
...
% valtozok.plot.savejpg=1; - (1/0) if .jpg file is to be saved
% valtozok.plot.savefig=0; - (1/0) if Matlab .fig file is to be saved
% 
% valtozok.gj_baselinelength=.010; % (seconds) - time before the start of 
%   the square pulse
% valtozok.gj_baselinelengthend=.08; (seconds) - time after the start of
%   the square pulse
% valtozok.gj_minlinelength=.05; (seconds) - minimum sweep time for
%   analysis. Shorter sweeps are discarded
% valtozok.gj_mincurrampl=-10*10^-12; (Ampere) - minimum current to be
%   injected. Smaller current steps are discarded.
% 
% valtozok.noAPbeforetheevent=1; (seconds) - traces where the postsynaptic
%   cell has an action potential within this timeframe are discarded (back 
%   in time relative to the presynaptic AP)
% valtozok.noAPaftertheevent=.05; (seconds) - traces where the postsynaptic
%   cell has an action potential within this timeframe are discarded (forward 
%   in time relative to the presynaptic AP)
% valtozok.pairedpulseneeded=0; % (0/1) - only paired pulses are saved
% valtozok.pairedpulsedelay=0.06; (seconds) 
% valtozok.pairedpulsejitter=.05; (seconds)
%                                           - only paired pulses with 
%   valtozok.pairedpulsedelay interspike interval +- valtozok.pairedpulsejitter
%   are saved. (works only if valtozok.pairedpulseneeded == 1)
% valtozok.baselinelength=0.025; (seconds) - baseline before first
%   presynaptic AP
% valtozok.psplength=.15; (seconds) - time saved after the (last) AP of 
%   each sweep (in case of paired pulses)
% valtozok.filterorder=3; 
% valtozok.cutofffreq=1000;
% valtozok.drugwashintime=120; (seconds) - time or a drug to reach the
%   chamber and be effective (in case of pharmacological experiments)
% valtozok.maxy0baselinedifference=.0005; (Volt) - in case of
%   pharmacological experiments, this is the maximal voltage difference
%   between baseline and drug conditions
% valtozok.discardpostsweepswithap=1; (1/0) - sweeps where the postsynaptic
%   cell fires are discarded
% valtozok.postrecordingmode='C-Clamp'; ('C-Clamp' or 'V-Clamp' or 'any')
% valtozok.prerecordingmode='C-Clamp'; ('C-Clamp' or 'V-Clamp' or 'any')
% See also: aE_testforGJ, aE_testforChemicalSynapse
