...
for xlsi=1:length(xlsdata)
    if any(strfind(xlsdata(xlsi).anaesthesia,'awake')) & xlsdata(xlsi).field==0 & xlsdata(xlsi).juxta==0
        ...
        if isempty(a)
            ...
            APdata(NEXT).aAPnum=sum([eventdata.axonalAP]);
            APdata(NEXT).sAPnum=sum([eventdata.somaticAP]);
        end
    end
end
%%
